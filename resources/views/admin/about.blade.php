    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="border rounded p-4 space-y-3">
            ...
            <div>
                ...
                <input type="file" name="cards[{{ $i }}][image_file]" accept="image/*" class="w-full text-sm text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary-brand file:text-white hover:file:bg-blue-700">
                <p class="text-xs text-slate-400 mt-1">PNG, JPG, WebP. Max 5MB. Kosongkan jika tidak ingin mengubah.</p>
            </div>
            @if(!empty($card['image']))
            <div>
                <img src="{{ $card['image'] }}" alt="Card {{ $i + 1 }}" class="h-24 object-cover rounded border">
            </div>
            @endif
        </div>
        @endforeach
