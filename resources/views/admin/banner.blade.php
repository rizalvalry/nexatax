...
@section('content')
<form action="{{ route('admin.banner.update') }}" method="POST" class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="url" name="slides[{{ $i }}][image]" value="{{ $slide['image'] ?? '' }}" class="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:border-primary-brand">
            </div>
        </div>
        @if(!empty($slide['image']))
        <img src="{{ $slide['image'] }}" alt="Preview" class="h-32 object-cover rounded mt-2">
        @endif
    </div>
    @endforeach
